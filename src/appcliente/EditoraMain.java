...
import static DAO.EditoraDAO.alterarEditora;
import static DAO.EditoraDAO.buscaEditoras;
import static DAO.EditoraDAO.detalharEditora;
import static DAO.EditoraDAO.inserirEditora;
 ...
 */
public class EditoraMain extends javax.swing.JInternalFrame {
    ...
    EditoraMain(String quemChamouExterno, String descricaoTD) {
        initComponents();
        lblResult.setText("");
        this.setEnabled(true);
        retornaDados = true;
    ...
    private void botaoPesquisarPressionado() {
        ...
        Collection<EditoraTO> lista = new ArrayList<>();
        try {
            lista = buscaEditoras(jTextPesquisar.getText().trim());
        } catch (DaoException ex) {
            Logger.getLogger(EditoraMain.class.getName()).log(Level.SEVERE, null, ex);
            JOptionPane.showMessageDialog(this, ex.getMessage() + " \nErro consulta ao BD.");
        }
        getTableModel().setRowCount(0);
        if (lista != null && lista.size() > 0) {
            lblResult.setText(lista.size() + " registro(s) encontrado(s)");
            for (EditoraTO v : lista) {
                String[] rowData = new String[2];
                rowData[0] = String.format("%04d", v.getIdEditora());
                rowData[1] = v.getNome();
    ...
    private void botaoOKPressionado() throws HeadlessException {
        ...
        if (fluxo.equals("ALTERAR")) {
            edt = parseTO(fluxo, Integer.valueOf(tblResult.getValueAt(tblResult.getSelectedRow(), 0).toString()));
            try {
                alterarEditora(edt);
    ...
    private EditoraTO parseTO(String fluxoAux, int ID) {
        EditoraTO edt = new EditoraTO();
        if (!fluxoAux.equals("INCLUIR")) {
            edt.setIdEditora(ID);
        }
        edt.setNome(jtxtNome.getText());
        return edt;
    }
    ...
    private void btnAlterarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAlterarActionPerformed
        try {
            if (tblResult.getSelectedRow() == -1) {
                JOptionPane.showMessageDialog(this, "Selecione Editora.");
                return;
            }
            bloqueiaBotoes();
            ...
            // Busca Vendedor
            EditoraTO to;
            to = detalharEditora(Integer.valueOf(tblResult.getValueAt(tblResult.getSelectedRow(), 0).toString()));

            if (to != null) {
                jtxtCodigo.setText(String.valueOf(to.getIdEditora()));
                jtxtNome.setText(to.getNome());
            }
            jtxtNome.requestFocus();

        } catch (DaoException ex) {
            Logger.getLogger(EditoraMain.class
    ...
    private void btnExcluirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnExcluirActionPerformed
        ...
        e = new EditoraTO();
        try {
            e = detalharEditora(Integer.valueOf(getCodigoSelecionado()));
        } catch (DaoException ex) {
            Logger.getLogger(EditoraMain.class.getName()).log(Level.SEVERE, null, ex);
            JOptionPane.showMessageDialog(this, ex.getMessage() + " \nErro na aplicacao");
        }
